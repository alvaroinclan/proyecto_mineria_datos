...
for col in num_cols:
    ...
    print(f"  {col:30s} {g_nan.mean():10.3f} {g_ok.mean():10.3f} "
          f"{p:12.4e} {sig:>7s}")
...
for i, col in enumerate(num_cols):
    axes[i].hist(df.loc[~mask_nan, col], bins=30, alpha=0.6, # ~ operador logico NOT para bool
                 color="#3498db", label="OK", density=True)
    axes[i].hist(df.loc[mask_nan, col], bins=30, alpha=0.6,
                 color="#e74c3c", label="NaN", density=True)
    axes[i].set_title(col)
    axes[i].legend(fontsize=8)
plt.suptitle("Distribuciones: filas con NaN vs sin NaN en addiction_level",
             fontsize=13, y=1.01)
plt.tight_layout()
...
ct = pd.crosstab(df["addiction_level"].fillna("(NaN)"),
                 df["addicted_label"], margins=True)
print("\n  Tabla cruzada:")
print(ct.to_string())

# Verificar determinismo
df_ok = df.dropna(subset=["addiction_level"])
mild_en_1 = ((df_ok["addiction_level"] == "Mild") &
...
ct_plot = pd.crosstab(df["addiction_level"].fillna("(NaN)"),
                      df["addicted_label"])
